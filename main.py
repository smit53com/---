...
def get_all_users() -> List[Dict[str, Any]]:
    with get_db() as conn:
        ...
        for row in rows:
            ...
            profile['daily_off'] = row['daily_off']
...
def draw_chart(positions: Dict[str, float], houses: List[float], 
    ...
    for name, lon in positions.items():
        ...
        offset = (idx - len(sector_list)/2) * 0.08 if len(sector_list) > 1 else 0
...
async def get_birth_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"{e}\nПопробуйте: `18.08.1995 14:30 Москва, Россия`", parse_mode="Markdown")
        return BIRTH_DATA
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "change":
        ...
        await query.message.reply_text(
            "Данные сброшены. Отправьте новые:\n`ДД.ММ.ГГГГ ЧЧ:ММ Город, Страна`",
            parse_mode="Markdown"
        )
        return
    ...
    if query.data in THEME_MAP:
        ...
        try:
            ...
            await query.message.reply_text("Не удалось получить анализ.", reply_markup=kb_topics())
...
async def free_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not profile:
        await update.message.reply_text("Сначала настройте профиль: /start", parse_mode="Markdown")
        return
    ...
    try:
        ...
        await update.message.reply_text("Не удалось обработать вопрос.", reply_markup=kb_topics())
...
Вы можете задавать любые вопросы текстом!
    """
...
async def daily_job(app: Application):
    ...
    for profile in users:
        ...
        try:
            ...
            if local_now.hour != 8 or local_now.minute >= 15:
                continue

            if profile.get('last_daily_sent') == today_str:
                logger.info(f"Пропуск: уже отправлено сегодня пользователю {uid}")
                continue
            ...
            text = gpt_analyze(
                ...
                "Используй слово 'Сегодня', не указывай дату. "
                "Сделай текст вдохновляющим и личным.",
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update and update.effective_message:
        await update.effective_message.reply_text("Произошла ошибка. Попробуйте позже или используйте /help")
